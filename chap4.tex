...
函数~\textsc{PlayCard}~（算法~\ref{alg:playCard}）用于产生使用手牌的决策。首先，在$5\sim 9$行，~\textsc{PlayCard}~会首先计算对方所有可攻击角色的攻击力总和，并将结果与我方英雄剩余生命值进行比较。
...
在$11\sim 17$行，~\textsc{PlayCard}~函数会选择手牌中可用的法力消耗最高的随从卡，并使用该卡。如果手牌中不存在任何可用的随从卡，~\textsc{PlayCard}~函数则会返回~null~，表示所有可进行的手牌使用已枚举完毕。
...
函数~\textsc{Attack}~（算法~\ref{alg:attack}）用于枚举随从与英雄的攻击动作。在第~5~行，~\textsc{Attack}~函数会首先获取所有可进行攻击的友方角色，并对其按攻击力进行降序排序。
第~$6\sim8$~行用于判断边界情况：当不存在任何可进行攻击的角色时，意味着~\textsc{Attack}~函数已完成所有角色攻击决策的枚举，因此返回~null~。
第~$9\sim 15$~行用于枚举所有可攻击的敌方目标。首先~\textsc{Attack}~函数会尝试获取可攻击的敌方嘲讽随从，若不存在，则尝试攻击敌方威胁较大的随从。
...
首先在第~$17\sim19$行，~\textsc{Attack}~函数会尝试获取一个该我方角色可以杀死的生命值最大的敌方随从，并指定我方角色攻击该目标。
...
\item \textsc{PlayCard}：随机规则智能体在进入~\textsc{PlayCard}~函数后，有 $50\%$ 的可能性使其随机地使用卡牌。具体的做法是在所有可用的手牌中随机选择一张并使用。
\item \textsc{Attack}：随机规则智能体在进入~\textsc{Attack}~函数后，有 $50\%$ 的可能性使其随机地为我方角色指派攻击目标。具体的做法是，首先被选中的我方角色有 $50\%$ 的可能选择不攻击；选择攻击时，攻击的目标也会随机地进行选择。
